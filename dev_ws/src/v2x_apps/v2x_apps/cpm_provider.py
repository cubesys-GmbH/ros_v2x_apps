...
class CpmProvider(Node):
    def __init__(self):
        ...
        self.pos_vector_subscription = self.create_subscription(PositionVector, '/its/position_vector', self.position_update, 1)
        
        # Publish who provides CPM to cube-its
        self.cpm_publisher = self.create_publisher(cpm_msg.CollectivePerceptionMessage, '/its/cpm_provided', 1)
    ...
    def generate_perceived_object_cpm(self) -> cpm_msg.CollectivePerceptionMessage:
        ...
        container.container_data.perceived_object_container.perceived_objects.array = [perceived_object]
