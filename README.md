...
The [*cam_listener*](https://github.com/cubesys-GmbH/ros_v2x_apps/blob/master/dev_ws/src/v2x_apps/v2x_apps/cam_listener.py), depicted in Figure 2, subscribes to CAMs received by *cube-its* and logs the sending station ID.

| Direction | Topic / Service | Type |
| --- | --- | --- |
| Subscribes | `/its/cam_received` | CAM |
...
The [*denm_node*](https://github.com/cubesys-GmbH/ros_v2x_apps/blob/master/dev_ws/src/v2x_apps/v2x_apps/denm_node.py), illustrated in Figure 3, periodically requests a DENM transmission based on the current position, and logs DENMs received from other stations.

| Direction | Topic / Service | Type |
| --- | --- | --- |
| Subscribes | `/its/position_vector` | own position |
| Subscribes | `/its/denm_received` | incoming DENMs |
| Calls service | `/its/den_request` | request DENM transmission |

### Collective Perception Message

![Figure 4 - Project cpm_provider](images/cpm_provider.png "Figure 4 - Project cpm_provider")

The [*cpm_provider*](https://github.com/cubesys-GmbH/ros_v2x_apps/blob/master/dev_ws/src/v2x_apps/v2x_apps/cpm_provider.py), illustrated in Figure 4, periodically generates a CPM containing a sample Perceived Object and publishes it for transmission by the CPS facility in *cube-its*.

| Direction | Topic / Service | Type |
| --- | --- | --- |
| Subscribes | `/its/position_vector` | own position |
| Publishes | `/its/cpm_provided` | CPM |
...
The [*vam_provider*](https://github.com/cubesys-GmbH/ros_v2x_apps/blob/master/dev_ws/src/v2x_apps/v2x_apps/vam_provider.py), shown in Figure 5, periodically generates a VAM with sample VRU data and publishes it for transmission by the VA facility in *cube-its*.

| Direction | Topic / Service | Type |
| --- | --- | --- |
| Subscribes | `/its/position_vector` | own position |
| Publishes | `/its/vam_provided` | VAM |
...
The [*stationary_vehicle*](https://github.com/cubesys-GmbH/ros_v2x_apps/blob/master/dev_ws/src/v2x_apps/c2c/stationary_vehicle_trigger.py), depicted in Figure 6, sends a single request to trigger (or terminate) a Stationary Vehicle Warning via the *cube-its* facility service. Once triggered, *cube-its* keeps transmitting the StVeWa message — a DENM profile defined by [C2C-CC](https://www.car-2-car.org/) — until a termination request is sent.

| Direction | Topic / Service | Type |
| --- | --- | --- |
| Calls service | `/c2c/stationary_vehicle_request` | trigger / terminate |
